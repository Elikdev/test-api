...
class PaymentDetailsServices extends BaseService {
 ...
 return this.internalResponse(true, payment_details, 200, 'Payment details retrieved');
 }
  ...
  paymentDetailsDTO: {
   ...
   frequency_amount: string;
     ...
     if (user.payment_details) {
      ...
      const updated_data = this.schema(Payment_detail).merge(payment_detail, update_details)
     ...
     const bank_details = await BanksOption.find(
      ...
      return this.internalResponse(true, {...updated_data, bank_name: bank_details.name}, 200, "Payment details updated");
     }
     ...
     const newData = JSON.stringify({ ...result, bankName: bank_details.name });
