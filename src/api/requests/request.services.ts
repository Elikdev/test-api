...
class RequestService extends BaseService {
  ...
  public async createRequest(
    ...
    if (!influencer) {
      return this.internalResponse(
        ...
        {},
        400,
        "Influencer does not exist or has been deleted"
      )
    }
    ...
    if (
      ...
      (!rate_dm || rate_dm < 0 || rate_dm == null)
    ) {
      return this.internalResponse(
        false,
        {},
        400,
        "Influencer DM rate not set. Please try again later"
      )
    }
    ...
    if (
      ...
      (!rate_shout_out || rate_shout_out < 0 || rate_shout_out == null)
    ) {
      return this.internalResponse(
        false,
        {},
        400,
        "Influencer Shout Out rate not set. Please try again later"
      )
    }
    ...
    const rate_amount =
      request_type == "direct message" ? rate_dm : rate_shout_out
    const isWalletBalanceOk =
      parseFloat(fan_wallet.ledger_balance) > parseFloat(`${rate_amount}`)
    if (!isWalletBalanceOk) {
      return this.internalResponse(
        false,
        {},
        400,
        "Not enough funds in your wallet to process this request"
      )
    }
    ...
    try {
      ...
      const transac = await transactionService.saveTransactionWithQueryRunner(
        queryRunner,
        {
          user,
      ...
      await queryRunner.manager.update(Wallet, fan_tran_wallet.id, {
        ledger_balance: new_fan_ledger_balance.toFixed(2),
      })
      await queryRunner.manager.update(Wallet, inflencer_tran_wallet.id, {
      ...
      const message = compileEjs({ template: "update-template" })({
        body: `A user just made a request for ${request_type} which expires in 72 hours🚀.
                `,
        name: `${
          Array.isArray(influencer.full_name.split(" "))
      ...
      const cronFunParam = async () => {
        ...
        if (cronFunctionRequest.status === "pending") {
          ...
          const x = rate_amount
          const fan_cron_wallet = await walletService.findWalletByUserId(
            user_id
          )
          const influencer_cron_wallet = await walletService.findWalletByUserId(
            influencerId
          )
          ...
          await getConnection()
            ...
            .set({
              ledger_balance: `${
                parseFloat(fan_cron_wallet.ledger_balance) + x
              }`,
            })
          ...
          await getConnection()
            ...
            .set({
              ledger_balance: `${
                parseFloat(influencer_cron_wallet.ledger_balance) - x
              }`,
            })
  ...
  public async respondToRequest(
    ...
    if (request.status === RequestStatus.CANCELLED) {
      return this.internalResponse(
        ...
        {},
        400,
        "Request cancelled by fan already"
      )
    }
    ...
    try {
      ...
      const influencer_balance =
        type == "decline"
          ? parseFloat(influencer_wallet.ledger_balance) -
            parseFloat(request.rate)
          : parseFloat(influencer_wallet.wallet_balance) +
            parseFloat(request.rate)

      await queryRunner.manager.update(Wallet, fan_wallet.id, {
        ledger_balance:
          type == "decline"
            ? fan_balance.toFixed(2)
            : fan_wallet.ledger_balance,
        wallet_balance:
          type == "accept" ? fan_balance.toFixed(2) : fan_wallet.wallet_balance,
      })
      await queryRunner.manager.update(Wallet, influencer_wallet.id, {
        ledger_balance:
          type == "decline"
            ? influencer_balance.toFixed(2)
            : influencer_wallet.ledger_balance,
        wallet_balance:
          type == "accept"
            ? influencer_balance.toFixed(2)
            : influencer_wallet.wallet_balance,
      })
      ...
      const updatedRequest = await queryRunner.manager.update(
        ...
        {
          status:
            type == "decline" ? RequestStatus.REJECTED : RequestStatus.ACCEPTED,
          reason: type == "decline" && reason,
        }
      )
      const resUpdatedRequest = await queryRunner.manager.findOne(Requests, {
        where: { id: request.id },
      })
      ...
      if (type === "accept") {
        ...
        await queryRunner.manager.update(Requests, request.id, {
          room: room,
        })

        // send mail notification to influencer/, text will be changed
        const message = compileEjs({ template: "update-template" })({
          body: `An Influencer with handle ${room.influencer.handle} has just accepted your request for ${request.request_type}🚀. 
                       Do well to check the app to confirm this.
                `,
          name: `${
            Array.isArray(room.fan.full_name.split(" "))
              ? room.fan.full_name.split(" ")[0]
              : room.fan.full_name
          }`,
        })
        const sent_mail = await sendEmail({
  ...
  public async cancelRequest(authUser: jwtCred, requestId: number) {
    ...
    if (request.fan.id != user_exists.id) {
      return this.internalResponse(
        ...
        {},
        400,
        "Can't cancel someone else's request"
      )
    }
    ...
    if (request.status === RequestStatus.ACCEPTED) {
      return this.internalResponse(
        ...
        {},
        400,
        "Requests once accepted cannot be cancelled"
      )
    }
    ...
    try {
      ...
      const fan_ledger_balance =
        parseFloat(fan_wallet.ledger_balance) + parseFloat(request.rate)
      const influencer_ledger_balance =
      ...
      return this.internalResponse(true, {}, 200, "Request cancelled")
    } catch (error) {
      await queryRunner.rollbackTransaction()
      await queryRunner.release()
      return this.internalResponse(
  ...
  public async saveRequestWithQueryRunner(
  ...
  public async shoutOutVideoInstance(
    video_url: string,
    request: Requests,
    fan: Fan,
  ...
  public async saveShoutOutVideos(SvDTO: {
    requestId: number
    video_url: string
  }) {
    ...
    if (request_exists.request_type !== "shout out") {
      return this.internalResponse(
        ...
        {},
        400,
        "Request type must be a shout out"
      )
    }
    ...
    if (!fan_exists) {
      return this.internalResponse(
        ...
        {},
        400,
        "Invalid request as fan does not exist"
      )
    }
    ...
    if (!influencer_exists) {
      return this.internalResponse(
        ...
        {},
        400,
        "Invalid request as influencer does not esist"
      )
    }

    //save the shout out video
    const new_shout_out_video = await this.shoutOutVideoInstance(
      video_url,
      request_exists,
      fan_exists,
      influencer_exists
    ...
    if (!saved_shout_out_video) {
      return this.internalResponse(
        ...
        {},
        400,
        "Failed to save the shout out video"
      )
    }

    const {
      fan: req_fan,
      influencer: req_influencer,
      ...requestData
    } = saved_shout_out_video.request
    const { password, email_verification, ...influencerData } =
      saved_shout_out_video.influencer
    const {
      password: fanPwrd,
      email_verification: fanEv,
      ...fanData
    } = saved_shout_out_video.fan

    const response = {
      ...saved_shout_out_video,
      request: requestData,
      fan: fanData,
    ...
    return this.internalResponse(
      ...
      response,
      200,
      "Shout out video saved successfully"
    )
  }
  ...
  public async getAllShoutOutVideos() {
    ...
    return this.internalResponse(
      true,
      { soVideos, total: count },
      200,
      "Shout out videos retrieved"
    )
  }
  ...
  public async allRequestsCount() {
    ...
    if (list.length > 0) {
      for (const req of list) {
        ...
        delete req.fan.password, delete req.influencer.password
      }
    }
